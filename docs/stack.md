...
This document separates the **current starter setup** from the **fixed standard stack** for cloned products. A fixed standard is the approved default when its capability is needed; it is not permission to install every package before a feature requires it.
...
| Zod | Runtime schemas | Forms, actions, URLs, webhooks, external data | Replacing domain behavior or client-only trust | Valibot only by explicit product decision |
| Zustand | Minimal global client state | Genuine cross-feature client state | Local component state, server data, or URL state | React context for small, static dependency injection |
| nuqs | Typed URL state | Filters, sorting, pagination, shareable views | Hidden or ephemeral component state | Next search params for simple server-only reads |
...
Zod, Zustand, nuqs, Stripe, and Resend are fixed standards but are **not currently listed in `package.json`**. Install one only when the cloned product has the matching requirement; isolate provider SDKs behind a feature service or repository.
...
| Payments | `[DEFINE]` | Whether payments are required; Stripe configuration and payment workflows |
| Transactional email | `[DEFINE]` | Whether email is required; Resend configuration and delivery workflows |
